...
namespace GoogleSheetsToUnity
{
    ...
    public partial class SpreadsheetManager
    {
        ...
        public static void Read(GSTU_Search search, UnityAction<GstuSpreadSheet> callback, bool containsMergedCells = false)
        {
            ...
            //search.sheetId = "1a7y0UXRaCnW1igvWSVHG0RGISPB8uOAgDYKlPZ3xzVk";
            ...
            //GameStateManager.Instance.m_loadingTxt = "Evaluting sheet ID";
            //GameStateManager.Instance.m_loadingBar = 0.15f;
        ...
        static IEnumerator Read(UnityWebRequest request, GSTU_Search search, bool containsMergedCells, UnityAction<GstuSpreadSheet> callback)
        {
            ...
            using (request)
            {
                //GameStateManager.Instance.m_loadingBar = 0.65f;
                //GameStateManager.Instance.m_loadingTxt = "Downloading Data";
                yield return request.SendWebRequest();

                if (request.result == UnityWebRequest.Result.ConnectionError || string.IsNullOrEmpty(request.downloadHandler.text) || request.downloadHandler.text == "{}")
                {
                    Debug.LogWarning("Unable to Retreive data from google sheets");
                    yield break;
                }
                //GameStateManager.Instance.m_loadingBar = 1f;
                //GameStateManager.Instance.m_loadingTxt = "Data Download Finished";
                //yield return CSVDownloader.DownloadData(AfterDownload);
